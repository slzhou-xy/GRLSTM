...
def splitRatingData(user_dict, train_ratio=0.7, test_ratio=0.2, shuffle_data_split=False, filter_unseen_samples=True):
    ...
    if filter_unseen_samples:
        valid_list = [Rating(u_map[rating[0]], i_map[rating[1]], rating[2]) for rating in tmp_valid_list if
                      rating[1] in train_item_set]

        test_list = [Rating(u_map[rating[0]], i_map[rating[1]], rating[2]) for rating in tmp_test_list if
                     rating[1] in train_item_set]
    else:
...
def cutLowFrequentData(rating_file, item_vocab=None, low_frequence=10, logger=None):
    ...
    if logger is not None:
        logger.info(
            "Totally {} interactions between {} user and {} items!".format(len(df), len(user_dict), len(item_dict)))
    ...
    while True:
        ...
        if logger is not None:
            logger.info("Remaining : {} interactions of {} users and {} items!".format(total_ratings, len(user_dict),
                                                                                       len(item_dict)))
        if flag1 and flag2:
...
def preprocess(rating_file, out_path, train_ratio=0.7, test_ratio=0.2, shuffle_data_split=True,
               filter_unseen_samples=True, low_frequence=10, logger=None):
    ...
    if logger is not None:
        logger.info(
            "{} {} for {:.1f} training, {:.1f} validation and {:.1f} testing!".format(str_is_shuffle, rating_file,
                                                                                      train_ratio,
    ...
    train_list, valid_list, test_list, u_map, i_map = splitRatingData(user_dict, train_ratio=train_ratio,
                                                                      test_ratio=test_ratio,
                                                                      shuffle_data_split=shuffle_data_split,
                                                                      filter_unseen_samples=filter_unseen_samples)

    if logger is not None:
        logger.debug("Spliting dataset is done!")
        logger.info(
            "Filtering unseen users and items ..." if filter_unseen_samples else "Not filter unseen users and items.")
        logger.info("{} users and {} items, where {} train, {} valid, and {} test!".format(len(u_map), len(i_map),
                                                                                           len(train_list),
                                                                                           len(valid_list),
                                                                                           len(test_list)))
...
if __name__ == "__main__":
    ...
    logger.info("load {} ratings for {} users, where min {} and max {} ratings!".format(train_rating_total,
                                                                                        len(train_user_dict),
