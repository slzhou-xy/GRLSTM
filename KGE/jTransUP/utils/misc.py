...
def projection_transR_pytorch(original, proj_matrix):  # (batch, k_dim)   (batch, k_dim * dim) k_dim可以与dim相同
...
def projection_transR_pytorch_batch(original, proj_matrix):  # original: (entity_total, k_dim)
    ent_embedding_size = original.shape[1]
    rel_embedding_size = proj_matrix.shape[1] // ent_embedding_size  # proj_matrix: (batch, k_dim * dim)
    proj_matrix = proj_matrix.view(-1, rel_embedding_size, ent_embedding_size)  # (batch, k_dim, dim)
    return torch.matmul(proj_matrix, original.transpose(0, 1)).transpose(1, 2)  # (batch, entity_total, k_dim)
...
def getKGPerformance(pred, gold, fliter_samples=None, topn=10):
    ...
    for rank_id in pred_ranks:
        if fliter_samples is not None and rank_id in fliter_samples:
            if current_rank < topn:  # 如果id已经出现在train/test中, 则跳过，继续往后遍历
                topn_to_skip += 1
            continue
        if rank_id in gold:  # 如果id是positive的,则预测准确
            gold_ranks.append(current_rank)  # 在第几位预测准确
            gold_ids.append(rank_id)  # 具体预测的是哪个h或t
            hits.append(1 if current_rank < topn else 0)
            if len(gold_ranks) == len(gold):  # 找到len(gold)个positive为止
                break
        else:
            current_rank += 1
