...
def getTrainTripleBatch(triple_batch, entity_total, all_head_dicts=None, all_tail_dicts=None):
    negTripleList = [corrupt_head_filter(triple, entity_total, headDicts=all_head_dicts) if random.random() < 0.5
                     else corrupt_tail_filter(triple, entity_total, tailDicts=all_tail_dicts) for triple in
                     triple_batch]
    # yield u, pi, ni, each list contains batch size ids,
    ph, pt, pr = getTripleElements(triple_batch)  # positive samples
    nh, nt, nr = getTripleElements(negTripleList)  # negative samples
...
def MakeTrainIterator(
        ...
        batch_size,
        negtive_samples=1):
    train_list = np.array(train_data)  # 将[(h, t, r), ...]转换为numpy矩阵[[h t r], ...]
    ...
    def data_iter():
        ...
        while True:
            ...
            # numpy->list
            yield train_list[batch_indices].tolist()  # batch_size个(h, t, r),即[[h, t, r], ...] batch_size个

    return data_iter()   # [[(h, t, r), ...], ...]  # 很多个batch_size
...
def MakeEvalIterator(
    ...
    eval_list = np.asarray(eval_data, data_type)  # [(t, r), ...]  # nd array
    ...
    while True:
        ...
        data_iter.append(candidate_batch.tolist())  # [(t, r), ...] list    len: batch_size

    return data_iter  # [[(t, r), ...], ...]
