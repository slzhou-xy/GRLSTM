...
class TransHModel(nn.Module):
    def __init__(self,
        ...
        ent_weight = torch.FloatTensor(self.ent_total, self.embedding_size)  # 参数权重为0
        rel_weight = torch.FloatTensor(self.rel_total, self.embedding_size)  # paper中的relation vector d_r
        norm_weight = torch.FloatTensor(self.rel_total, self.embedding_size)  # paper中的norm vector w_r
        nn.init.xavier_uniform(ent_weight)  # 给ent_weight初始化参数权重
        nn.init.xavier_uniform(rel_weight)
        nn.init.xavier_uniform(norm_weight)
        # init user and item embeddings
        self.ent_embeddings = nn.Embedding(self.ent_total, self.embedding_size)  # 随机初始化参数
        ...
        self.ent_embeddings.weight = nn.Parameter(ent_weight)  # 用ent_weight中参数来初始化self.ent_embedding中的权重
        self.rel_embeddings.weight = nn.Parameter(rel_weight)
        self.norm_embeddings.weight = nn.Parameter(norm_weight)

        normalize_ent_emb = F.normalize(self.ent_embeddings.weight.data, p=2, dim=1)  # 归一化
