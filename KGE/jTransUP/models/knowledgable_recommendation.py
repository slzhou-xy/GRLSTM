...
def evaluateRec(FLAGS, model, eval_iter, eval_dict, all_dicts, i_map, logger, eval_descending=True, is_report=False):
    ...
    for u_ids in eval_iter:
        ...
        results.extend(evalRecProcess(list(preds), eval_dict, all_dicts=all_dicts, descending=eval_descending,
    ...
    logger.info(
        "f1:{:.4f}, p:{:.4f}, r:{:.4f}, hit:{:.4f}, ndcg:{:.4f}, topn:{}.".format(f1, p, r, hit, ndcg, FLAGS.topn))
    ...
    if is_report:
        ...
        for pred_tuple in predict_tuples:
            ...
            if FLAGS.model_type in ["transup", "jtransup"]:
                ...
                gold_strs = ",".join(
                    ["{}({})".format(ir[0], ir[1]) for ir in zip(gold_ids, max_rel_index.data.tolist())])
            else:
...
def evaluateKG(FLAGS, model, eval_head_iter, eval_tail_iter, eval_head_dict, eval_tail_dict, all_head_dicts,
               all_tail_dicts, e_map, logger, eval_descending=True, is_report=False):
    # Evaluate
...
def train_loop(FLAGS, model, trainer, rating_train_dataset, triple_train_dataset, rating_eval_datasets,
               triple_eval_datasets, e_map, i_map, ikg_map, logger, vis=None, is_report=False):
    ...
    for _ in range(trainer.step, FLAGS.training_steps):
        ...
        if 0 < FLAGS.early_stopping_steps_to_wait < (
                trainer.step - trainer.best_step):
            ...
            if pbar is not None:
                pbar.close()
            break
        if trainer.step % FLAGS.eval_interval_steps == 0 and (
                len(rating_eval_datasets) > 0 or len(triple_eval_datasets) > 0):
            if pbar is not None:
                pbar.close()
            ...
            for i, eval_data in enumerate(rating_eval_datasets):
                all_eval_dicts = None
                if FLAGS.filter_wrong_corrupted:
                    all_eval_dicts = [rating_train_dict] + [tmp_data[3] for j, tmp_data in
                                                            enumerate(rating_eval_datasets) if j != i]

                rec_performances.append(
                    evaluateRec(FLAGS, model, eval_data[0], eval_data[3], all_eval_dicts, i_map, logger,
                                eval_descending=True if trainer.model_target == 1 else False, is_report=is_report))
            ...
            for i, eval_data in enumerate(triple_eval_datasets):
                ...
                if FLAGS.filter_wrong_corrupted:
                    eval_head_dicts = [head_train_dict] + [tmp_data[4] for j, tmp_data in
                                                           enumerate(triple_eval_datasets) if j != i]
                    eval_tail_dicts = [tail_train_dict] + [tmp_data[5] for j, tmp_data in
                                                           enumerate(triple_eval_datasets) if j != i]

                kg_performances.append(
                    evaluateKG(FLAGS, model, eval_data[0], eval_data[1], eval_data[4], eval_data[5], eval_head_dicts,
                               eval_tail_dicts, e_map, logger, eval_descending=False, is_report=is_report))
...
def run(only_forward=False):
    ...
    rating_train_dataset, rating_eval_datasets, u_map, i_map, triple_train_dataset, triple_eval_datasets, e_map, r_map, ikg_map = load_data(
        dataset_path, rec_eval_files, kg_eval_files, FLAGS.batch_size, negtive_samples=FLAGS.negtive_samples,
        logger=logger)
    ...
    joint_model = init_model(FLAGS, user_total, item_total, entity_total, relation_total, logger, i_map=i_map,
                             e_map=e_map, new_map=ikg_map)
    ...
    if FLAGS.load_ckpt_file is not None and FLAGS.share_embeddings:
        ...
        for filename in load_ckpt_files:
            trainer.loadEmbedding(os.path.join(FLAGS.log_path, filename), joint_model.state_dict(), e_remap=e_map,
                                  i_remap=i_map)
    ...
    if only_forward:
        for i, eval_data in enumerate(rating_eval_datasets):
            all_dicts = None
            if FLAGS.filter_wrong_corrupted:
                all_dicts = [rating_train_dict] + [tmp_data[3] for j, tmp_data in enumerate(rating_eval_datasets) if
                                                   j != i]
            evaluateRec(
                FLAGS,
                joint_model,
        ...
        for i, eval_data in enumerate(triple_eval_datasets):
            ...
            all_tail_dicts = None
            if FLAGS.filter_wrong_corrupted:
                all_head_dicts = [triple_train_head_dict] + [tmp_data[4] for j, tmp_data in
                                                             enumerate(triple_eval_datasets) if j != i]
                all_tail_dicts = [triple_train_tail_dict] + [tmp_data[5] for j, tmp_data in
                                                             enumerate(triple_eval_datasets) if j != i]
            evaluateKG(
                FLAGS,
                joint_model,
